...
**问题原因：**
后端返回的数据使用下划线命名（如 `student_id`），但前端期望驼峰命名（如 `studentId`）。在获取用户列表时没有进行字段名转换，导致学号列无法正确显示。

**修改内容：**

在 `getUserList` 函数中添加字段名转换逻辑：
...
**修改说明：**
- 将后端返回的 `student_id` 转换为前端的 `studentId`
- 同时转换其他字段以保持一致性
- 与审核列表页面（AuditList.vue）的处理方式保持一致
...
**注意事项：**
- 刷新浏览器页面即可看到效果
- 其他管理页面（如 PostManage.vue、CommentManage.vue）也可能存在类似问题，建议一并检查

---

### 2026-03-23 修复审核日志导出功能404错误

**问题：** 点击导出审核日志按钮时前端报404错误：`GET http://localhost:5173/api/admin/audit/export 404 (Not Found)`

**更新位置：**
1. `backend/api-gateway/routes/admin.js` - 添加导出审核日志接口
2. `frontend/src/utils/request.js` - 修复响应拦截器处理文件下载请求

**问题原因：**
1. 前端调用 `/admin/audit/export` 接口，但后端没有实现这个接口
2. 前端响应拦截器对所有响应都检查 `code` 字段，但文件下载请求返回的是二进制数据（blob），没有 `code` 字段，导致拦截器报错
...
router.get('/audit/export', async (req, res, next) => {
  try {
    const logs = await PostModel.getAuditLog({ ...req.query, pageSize: 10000 });
    
    const data = logs.list || [];
    const exportData = data.map(item => ({
      '日志ID': item.id,
      '帖子ID': item.post_id,
      '帖子内容': item.content || '',
      '审核类型': item.type === 1 ? 'AI审核' : item.type === 2 ? '人工审核' : '未知',
      '审核结果': item.result === 1 ? '通过' : item.result === 2 ? '驳回' : '未知',
      '审核人': item.operator_name || '系统',
      '审核人学号': item.operator_student_id || '',
    ...
    xlsx.utils.book_append_sheet(workbook, worksheet, '审核日志');
    
    const buffer = xlsx.write(workbook, { type: 'buffer', bookType: 'xlsx' });
    
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename=审核日志_${new Date().toLocaleDateString()}.xlsx`);
    res.send(buffer);
  } catch (err) { next(err); }
});
```
...
**修改说明：**
1. 后端新增 `/admin/audit/export` 接口，使用 xlsx 库生成 Excel 文件
2. 导出的数据包括：日志ID、帖子ID、帖子内容、审核类型、审核结果、审核人、审核人学号、驳回原因、创建时间
3. 前端响应拦截器增加对文件下载请求的特殊处理，避免对 blob 类型的响应进行 code 检查
4. 导出的 Excel 文件名为 `审核日志_日期.xlsx`

**验证结果：**
- 点击导出按钮可以成功下载 Excel 文件
- 文件包含所有审核日志数据（最多10000条）
- 文件格式正确，可以使用 Excel 打开
- 前端不再报404错误
...
### 2026-03-24 修复导出日志时Content-Disposition头部编码错误

**问题：** 导出审核日志时前端报500错误，后端报错：`Invalid character in header content ["Content-Disposition"]`

**更新位置：**
`backend/api-gateway/routes/admin.js`

**问题原因：**
HTTP 头部只支持 ASCII 字符，不能直接包含中文字符。之前的代码在 `Content-Disposition` 头部中直接使用了中文文件名，导致后端报错。

**修改内容：**

使用 `encodeURIComponent` 对文件名进行编码，并采用标准的 RFC 2231 格式：

```javascript
const buffer = xlsx.write(workbook, { type: 'buffer', bookType: 'xlsx' });

const filename = `审核日志_${new Date().toLocaleDateString()}.xlsx`;
...
**修改说明：**
1. 使用 `encodeURIComponent` 对中文文件名进行 URL 编码
2. 采用 RFC 2231 标准格式：`filename*=UTF-8''encoded_filename`
3. 同时保留传统的 `filename` 参数以兼容旧版浏览器
4. 确保文件名在下载时能正确显示中文
